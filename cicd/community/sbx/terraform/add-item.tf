...
# Additional item for User Service Table with tenantCode set to "local"
resource "aws_dynamodb_table_item" "user_service_table_item2" {
  count      = var.add_item ? 1 : 0
  depends_on = [module.lms_backend]
  table_name = var.user_service_table_name
  hash_key   = "pk"
  ...
  item = <<ITEM
  {
    "pk": {"S": "e92e1095-d380-4cf1-8210-39912f96b17h"},
    "sk": {"S": "acad4b4b-1db7-4a9d-87f9-96f7db9853e1"},
    "tenantCode": {"S": "local"},
    "createdOn": {"S": "2/17/2026  4:15:00 AM"},
    "gsiSortFNLN": {"S": "Superadmin"},
    "status": {"S": "Active"},
    "userType": {"S": "Internal"},
    "institutionName": {"S": "Cognizant"},
    ...
    "lastName": {"S": "Admin"},
    "role": {"S": "super-admin,learner,system-admin"},
    "emailId": {"S": "krishnakumar.vs2@cognizant.com"},
    "name": {"S": "superadmin"},
    "userAccountExpiryDate": {"S": "12/21/2035"},
    "lastLoginTimestamp": {"S": "2025-01-24T10:16:00Z"},
    "modifiedBy": {"S": "system"},
    "modifiedOn": {"S": "3/25/2025  0:00:00 AM"}
  }
  ITEM
