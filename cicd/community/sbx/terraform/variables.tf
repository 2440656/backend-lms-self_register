...
variable "ecs_service_name" {
  ...
  type        = string
  default     = "lms-course-enrolment-service"
}

variable "ecs_imageversion" {
  description = "The image version containing the ECS deployment package"
  type        = string
}
...
variable "lb_listner_port" {
  description = "Load balancer listener port exposed for accept traffic on load balancer"
  type        = number
  default     = 8080
}
...
variable "dd_api_key_secret_arn" {
  description = "(Optional) Secrets Manager ARN containing the Datadog API key. If set, exposed to container as DD_API_KEY secret."
  type        = string
  default     = ""
}

variable "additional_secrets" {
  description = "Optional list of additional secrets (objects with name and valueFrom)."
  type = list(object({
    name      = string
    valueFrom = string
  }))
...
// vpc environment varibale can be map to existing vpc varibale and below varibales can be deleted
variable "existing_vpc_id_1" {
  description = "Existing VPC ID"
  type        = string
}

variable "existing_private_subnets_1" {
  description = "Private subnet IDs for ECS tasks and internal ALB"
  type        = list(string)
}

variable "existing_security_group_ids_1" {
  description = "Existing security group IDs to attach to ECS tasks"
  type        = list(string)
}
...
variable "health_check_path" {
  description = "HTTP path that ALB uses for health checks"
  type        = string
  default     = "/health"
}
...
variable "execution_role_arn" {
  description = "ARN of Task execution role allows ECS to pull image from ECR and publish logs"
  type        = string
}